...
// Pemanggilan fungsi dengan async/await
// Menampilkan hasil


async function igdl333(url) {
  ...
  try {
    ...
    const results = await page.evaluate(() => {
      ...
      if (!outputList) {
        return { error: 'Output list not found' };
      }
    ...
    console.log('Results:', JSON.stringify(results, null, 2)); // Menampilkan hasil di konsol dalam format JSON
    return results;
  } catch (error) {
...
async function tiktok(url) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // 🔥 TAMBAHAN AMAN (TIDAK NGERUSAK)
      result.filename = `tiktok_${result.author || "user"}_${Date.now()}.mp4`;
