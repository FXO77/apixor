...
const checkAuthad = (req, res, next) => {
  if (!req.session.username) {
    return res.redirect('/asu');
  }
  next();
};
...
router.post('/asu', (req, res) => {
  ...
  db.query('SELECT * FROM users WHERE username = ? AND password = ?', [username, password], (err, result) => {
    ...
    if (result.length > 0) {
      ...
      res.send(`<script>alert('Username atau Password salah!'); window.location.href = '/asu';</script>`);
    }
  });
});
//coo
